...
class Handle(object):
    ...
    def clear(self):
        ...
        self.tree = ET.ElementTree() # pylint: disable=no-member
        self.img_src = {}
        self.img_files = {}
        self.page_src = {}
        self.page_files = {}
...
class Initializer(object):
    ...
    def _load_or_create_page(self):
        ...
        """
        page_fileGrps = self.handle.tree.getroot().findall(
            ".//mets:fileGrp[@USE='FULLTEXT']", ns)
        # case load page
        if page_fileGrps:
            for page_fileGrp in page_fileGrps:
                ...
                for page_file in page_files:
                    ...
                    page_url = page_file.find("mets:FLocat", ns).get(
                        "{%s}href" % ns["xlink"])
                    ...
                    if page_data.status_code == 200:
                        ...
                        self.handle.page_files[ID] = "%s/%s" % (
                            self.working_dir, os.path.basename(page_url))
                        with open(self.handle.page_files[ID], 'wb') as f:
                            f.write(page_data.content)
                        # parse the page xml (and store the tree to avoid repeated parsing)
                        self.handle.page_trees[ID] = ET.ElementTree()  # pylint: disable=no-member
                        self.handle.page_trees[ID].parse(
                            self.handle.page_files[ID])
    ...
    def _load_images(self):
        ...
        """
        img_fileGrps = self.handle.tree.getroot().findall(
            ".//mets:fileGrp[@USE='IMAGE']", ns)
        for img_fileGrp in img_fileGrps:
            ...
            for img_file in img_files:
                ...
                img_url = img_file.find("mets:FLocat", ns).get(
                    "{%s}href" % ns["xlink"])
                ...
                if img_data.status_code == 200:
                    ...
                    self.handle.img_files[ID] = "%s/%s" % (
                        self.working_dir, os.path.basename(img_url))
