...
def _fixture():
    mets = OcrdMets(filename=assets.url_of(
        'SBB0000F29300010000/data/mets.xml'), cache_flag=True)
    yield mets


def test_unique_identifier():
    mets = OcrdMets(filename=assets.url_of('SBB0000F29300010000/data/mets.xml'), cache_flag=True)
...
def test_unique_identifier_from_nothing():
    mets = OcrdMets.empty_mets(datetime.now().isoformat(), cache_flag=True)
...
def test_str():
    mets = OcrdMets(content='<mets/>', cache_flag=True)
...
def test_override_constructor_args():
    id2file = {'foo': {}}
    mets = OcrdMets(id2file, content='<mets/>', cache_flag=True)
...
def _fixture_sbb(tmp_path):
    ...
    yield OcrdMets(filename=mets_path, cache_flag=True)
...
def test_add_group():
    mets = OcrdMets.empty_mets(cache_flag=True)
