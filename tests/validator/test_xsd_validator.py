...
from ocrd_validators import XsdValidator, XsdMetsValidator
from ocrd_validators.constants import XSD_METS_URL
...
class TestXsdPageValidator(TestXsdValidator):

    def test_validate_simple_protected_doc(self):
        val = XsdPageValidator()
        report = val._validate(self.ws.page._tree)
        self.assertTrue(report.is_valid)

    def test_validate_simple_static_doc(self):
        report = XsdValidator.validate(XSD_PAGE_URL, self.ws.page._tree)
        self.assertTrue(report.is_valid)
