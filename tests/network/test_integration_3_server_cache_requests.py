...
def create_processing_jobs_db_entries(
    ...
    for processing_request in processing_requests_list:
        ...
        jobs_list.append(db_processing_job)
    return jobs_list
...
def test_is_caching_required():
    requests_cache = CacheProcessingRequests()
    workspace_key = "/path/to/mets.xml"
    jobs_list = create_processing_jobs_db_entries(requests_cache=requests_cache, workspace_key=workspace_key)

    # depends on nothing, should not be cached
    assert not requests_cache.sync_is_caching_required(job_dependencies=jobs_list[0].depends_on)
    # depends on processing_job_1, should be cached
    assert requests_cache.sync_is_caching_required(job_dependencies=jobs_list[1].depends_on)
    # depends on processing_job_2, should be cached
    assert requests_cache.sync_is_caching_required(job_dependencies=jobs_list[2].depends_on)
    # depends on processing_job_2, should be cached
    assert requests_cache.sync_is_caching_required(job_dependencies=jobs_list[3].depends_on)

    sync_db_update_processing_job(jobs_list[0].job_id, state=JobState.success)
    # the dependent job has successfully finished, no caching required
    assert not requests_cache.sync_is_caching_required(job_dependencies=jobs_list[1].depends_on)
    sync_db_update_processing_job(jobs_list[1].job_id, state=JobState.success)
    # the dependent job has successfully finished, no caching required for job 3 and job 4
    assert not requests_cache.sync_is_caching_required(job_dependencies=jobs_list[2].depends_on)
    assert not requests_cache.sync_is_caching_required(job_dependencies=jobs_list[3].depends_on)
...
def test_cancel_dependent_jobs():
    ...
    db_processing_job_1 = sync_db_update_processing_job(jobs_list[0].job_id, state=JobState.failed)
    assert db_processing_job_1.state == JobState.failed
    requests_cache.sync_cancel_dependent_jobs(workspace_key=workspace_key, processing_job_id=jobs_list[0].job_id)
    db_processing_job_2 = sync_db_get_processing_job(job_id=jobs_list[1].job_id)
    db_processing_job_3 = sync_db_get_processing_job(job_id=jobs_list[2].job_id)
    db_processing_job_4 = sync_db_get_processing_job(job_id=jobs_list[3].job_id)
...
def test_consume_cached_requests():
    ...
    db_processing_job_1 = sync_db_update_processing_job(jobs_list[0].job_id, state=JobState.success)
    ...
    db_processing_job_2 = sync_db_update_processing_job(jobs_list[1].job_id, state=JobState.success)
