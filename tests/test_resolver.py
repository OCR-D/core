...
def test_resolve_mets_arguments():
    ...
    """
    resolver = Resolver()
    assert resolver.resolve_mets_arguments(None, None, None, None, None, None) == (str(Path.cwd()), str(Path.cwd() / 'mets.xml'), 'mets.xml', None, None, None)
    assert resolver.resolve_mets_arguments('/', None, 'mets.xml', None, None, None) == ('/', '/mets.xml', 'mets.xml', None, None, None)
    assert resolver.resolve_mets_arguments('/foo', '/foo/foo.xml', None, None, None, None) == ('/foo', '/foo/foo.xml', 'foo.xml', None, None, None)
    assert resolver.resolve_mets_arguments(None, '/foo/foo.xml', None, None, None, None) == ('/foo', '/foo/foo.xml', 'foo.xml', None, None, None)
    assert resolver.resolve_mets_arguments('/foo', 'foo.xml', None, None, None, None) == ('/foo', '/foo/foo.xml', 'foo.xml', None, None, None)
    assert resolver.resolve_mets_arguments('/foo', 'http://bar/foo.xml', None, None, None, None) == ('/foo', 'http://bar/foo.xml', 'foo.xml', None, None, None)
    with pytest.raises(ValueError, match="Use either --mets or --mets-basename, not both"):
        resolver.resolve_mets_arguments('/', '/foo/bar', 'foo.xml', None, None, None)
    with pytest.raises(ValueError, match="inconsistent with --directory"):
        resolver.resolve_mets_arguments('/foo', '/bar/foo.xml', None, None, None, None)
    with pytest.warns(DeprecationWarning):
        resolver.resolve_mets_arguments('/foo', None, 'not_mets.xml', None, None, None)
    with pytest.raises(ValueError, match=r"--mets is an http\(s\) URL but no --directory was given"):
        resolver.resolve_mets_arguments(None, 'http://bar/foo.xml', None, None, None, None)
